...
let createAndInsert = function (auth) {

  console.log("Inside CreateAndInsert");
    ...
    fields: 'id'
      ...
      promiseObj.then( () => {
        fs.readdir(outputPath, (err, files) => {
          files.forEach(file => {
            var fileNamePath = outputPath + '/' +file;
            console.log("Encrypted Loc : ", fileNamePath)
            // Insert encrypted file into folder.
            var fileMetadata = {
                'name': `${file}`,
                parents: [folderID]
            };
 ...
 */
let authorize = function (credentials, callback) {
 ...
 */
let getAccessToken = function (oAuth2Client, callback) {
